...
App::App(int width, int height, const char* filename) :
    model_(filename), pos_(glm::rotate(glm::mat4(1.f), 70.f, {-1, 0, 0})) {
    if (!model_.valid()) {
        throw std::runtime_error("cannot load model!!!");
    }
    glm::mat4 projection = glm::perspective(
        glm::radians(90.0f), static_cast<float>(width) / height, 0.1f, 4000.0f);
    glm::mat4 view = glm::lookAt(
        glm::vec3(0.f, 0.0f, 60.f), glm::vec3(0, 40.f, 0), glm::vec3(0, 1, 0));

    neat::Model::setVP(view, projection);
